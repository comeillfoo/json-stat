...
fn accept_cb(expected: char) -> impl FnOnce(&str) -> Result<&str, &str> {
    move |stream: &str| accept(stream, expected)
}
...
fn accept_ignoring_case_cb(expected: char) -> impl FnOnce(&str) -> Result<&str, &str> {
    move |stream: &str| accept_ignoring_case(stream, expected)
}
...
pub fn single_json(file: &String) -> Result<bool, ParseError> {
    match fs::read_to_string(file) {
        Ok(content) => match accept_value(&content) {
            Ok(_) => Ok(true),
            Err(_) => Ok(false)
        },
